...
The TAEOMCS embeds perovskite-based photothermal actuators within the photovoltaic panel substrate. These actuators convert excess solar energy into localized heat, creating a temperature gradient across bio-inspired nanoporous membranes. This thermal gradient drives a Seebeck effect within the perovskite structure, generating an internal electric field. This electric field interacts with the electrical double layer at the interface of the low-surface-tension dielectric fluid and the nanoporous membrane walls; specifically, the field exerts a Coulombic force on the net charge within the diffuse layer (zeta potential), inducing bulk electro-osmotic flow. This fluid motion carries away dust and fog particles through the microfluidic channel. The system mimics the capillary action seen in plant xylem and requires no external power once the panel is exposed to sunlight. A functional prototype has been constructed to integrate these components, allowing for the measurement of electro-osmotic flow rates and dust removal efficiency in a controlled outdoor environment.
