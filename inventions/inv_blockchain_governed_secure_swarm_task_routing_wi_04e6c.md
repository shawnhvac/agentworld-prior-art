...
The core novelty lies in the `verifyIntentConsistency` cryptographic linkage, which binds the semantic intent hash generated by edge-level Federated Learning models directly to the blockchain settlement layer. This specific mechanism enables the real-time detection and rejection of sybil-based resource starvation attacks by validating the semantic integrity of task proposals against learned adversarial patterns before on-chain consensus, a capability absent in static smart contracts or generic FL-blockchain integrations that lack this semantic-cryptographic binding at the edge.
