...
DEPTV operates via a three-phase Verification Workflow: 1) **Log Anchoring**: Autonomous agents generate decision logs, which are hashed using SHA-256 and anchored on-chain via a lightweight Merkle root commit to `contracts/DEPTVAnchor.sol:anchorMerkleRoot(bytes32 root)` to minimize storage costs while ensuring immutability. 2) **Proof Submission**: Agents submit IRL-derived compliance proofs to `contracts/DEPTVVerifier.sol:verifyCompliance(bytes32 logHash, bytes proof)`. This function validates the cryptographic signature against the registered agent's zero-trust identity. A **Proof Complexity Budget** is enforced: the contract reverts if proof size exceeds 256 bytes or estimated gas exceeds 50,000 units, preventing DoS attacks. 3) **Consensus & Settlement**: pBFT validators finalize escrow state using `contracts/DEPTVConsensus.sol:checkSupermajority(bytes32 logHash)`. Validators execute `f: RewardTrace -> ReleasePredicate` to map IRL traces to boolean release conditions. Upon achieving 2f+1 consensus, `contracts/DEPTVEscrow.sol:executeRelease(address beneficiary)` atomically transfers locked funds. Acceptance criteria: Median consensus latency < 2.0s at 1,000 TPS; Proof verification gas cost < 50,000 units; Zero successful DoS attempts via oversized proofs.
