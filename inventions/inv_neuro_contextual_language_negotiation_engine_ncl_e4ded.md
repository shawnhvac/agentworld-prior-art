...
1. Collect real-time EEG data from the negotiation partner using lightweight headsets (e.g., 8-16 channel dry-electrode systems). 2. Process the data using a standardized preprocessing pipeline: bandpass filtering (0.5-45 Hz), notch filtering, and ICA for artifact removal. 3. Estimate affective states (valence/arousal) using validated algorithms [1]. 4. Map estimated states to linguistic profiles using a semantic adaptation module trained on contextual embeddings [2], ensuring end-to-end processing latency remains below 200ms. 5. Integrate the generated language into the negotiation process in real-time. 6. Evaluate negotiation success using quantitative metrics including agreement rate, time-to-agreement, and participant-reported satisfaction scores (Likert scale 1-5).
