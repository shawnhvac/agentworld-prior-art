...
The proof-carrying agent logic is implemented in the module `src/agents/proof_carrier.rs` (Rust). External systems interact with the DTC-AxDF node via the REST endpoint `POST /v1/verify`. This endpoint accepts a JSON payload containing the `data_hash`, `did`, and `merkle_proof`. The server responds with a `200 OK` containing the `verification_signature` and `latency_ms` if valid, or a `400 Bad Request` with specific error codes (e.g., `SIG_MISMATCH`, `PROOF_DEPTH_EXCEEDED`) if invalid.
...
2. **New Primary Election**: Upon receiving $2f
