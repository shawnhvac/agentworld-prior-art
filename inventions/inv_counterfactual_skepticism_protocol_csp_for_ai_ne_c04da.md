...
CSP is distinguished from recent LLM-based negotiation agents like AutoNegotiator [9] and ChatDev [10] by introducing a differentiable, term-level skepticism mechanism absent in their static, single-pass reasoning architectures. Specifically, while [9] and [10] optimize for immediate utility or global coherence without validating individual terms against adversarial scenarios, CSP integrates Gumbel-Softmax relaxation within a GenIR-based framework [2] to enable differentiable counterfactual sampling. This allows for localized, proposal-level robustness scoring via MCTS, which explicitly mitigates cognitive narrowing by enforcing discrete term-level logical consistency. Unlike global adversarial training which requires O(M * D) complexity for model-wide updates [3, 4], CSP’s localized approach operates with O(S * T) complexity during inference, providing granular defense against specific term vulnerabilities without the prohibitive cost of global weight updates. This results in a system that not only identifies high-risk liabilities (e.g., lock-in clauses under price volatility) but does so through a mathematically grounded, differentiable validation process that static frameworks [7, 8] cannot replicate.
