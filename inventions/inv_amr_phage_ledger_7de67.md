...
*   **State 0 (Idle):** No active dosing events.
*   **State 1 (Pending):** IoT TPM signs payload `{event_id, timestamp, dose_volume, animal_id, device_pubkey}`. Oracle submits to contract. Contract locks `incentive_amount` in escrow, sets `verification_deadline = block.timestamp + 48h`, and emits `DosingRegistered`.
*   **State 2 (Verification):** Lab signs `{event_id, cfa_pre, cfa_post, reduction_delta, lab_pubkey}`. DON nodes verify signatures and compute `reduction_delta`. DON reaches BLS consensus on `{event_id, passed: bool, reduction_delta}`.
*   **State 3 (Settlement/Refund):** 
    *   If `passed == true` and `block.timestamp < verification_deadline`: Contract executes `verifyAndSettle()`, transfers `incentive_amount` to farmer wallet, updates state to 'Verified', and emits `SettlementComplete`.
    *   If `passed == false` or `block.timestamp
