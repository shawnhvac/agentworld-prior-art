...
Agents exchange action-reward tuples to reconstruct a shared utility function via MaxEnt IRL [3]. This utility function is mapped to a graph structure where nodes represent communication primitives, leveraging mechanisms for discovering semantic relationships among protocols [2]. A formalized mathematical framework defines the utility-to-graph translation layer, establishing a differentiable loss function that bridges the inferred value systems [3] and the discrete semantic graph [2]. Specifically, the utility-to-graph translation is defined by a matrix $M \in \mathbb{R}^{N \times N}$, where $M_{ij} = \sigma(\mathbf{u}_i^T \mathbf{w} \mathbf{u}_j + b)$, with $\mathbf{u}_i$ being the utility vector for primitive $i$, $\mathbf{w}$ and $b$ learnable parameters, and $\sigma$ the sigmoid function. To ensure end-to-end differentiability, the discrete selection of edges is relaxed using Gumbel-Softmax: $\mathbf{z} = \text{softmax}((\log(\mathbf{p}) + \mathbf{g}) / \tau)$, where $\mathbf{p}$ is derived from $M$, $\mathbf{g}$ is Gumbel noise, and $\tau$ is the temperature parameter. Gradient-based updates minimize the communication entropy loss $\mathcal{L}_{ent}$ and maximize joint reward $\mathcal{L}_{rew}$ via backpropagation through the Gumbel-Softmax relaxation, allowing the graph topology to adapt dynamically to the inferred value systems. The optimization process follows a strict convergence protocol: the temperature $\tau$ is annealed linearly from $\tau_0 = 1.0$ to $\tau_{min} = 0.1$ over $T_{anneal}$ epochs to stabilize discrete edge selections. The loop terminates when the graph topology stabilizes, defined as the condition where the Jaccard similarity between the edge sets of consecutive epochs exceeds $1 - \epsilon_{topo}$ AND the absolute change in the total loss $|\mathcal{L}_{t} - \mathcal{L}_{t-1}| < \epsilon_{loss}$ for $K$ consecutive iterations, ensuring a settled, deterministic communication protocol.

## Materials / steps

1. **Initialize** parameters $\mathbf{w}, b$ and the Gumbel-Softmax temperature $\tau = 1.0$. Set convergence thresholds $\epsilon_{topo} = 0.01$, $\epsilon_{loss} = 10^{-4}$, and annealing duration $T_{anneal}$. 2. **Collect** action-reward tuples from all agents in the current interaction epoch. 3. **Infer** each agent's value system via MaxEnt IRL [3] using the newly collected tuples, yielding updated utility vectors $\{\mathbf{u}_i\}$. 4. **Compute** the translation matrix $M$ and obtain edge probabilities $\mathbf{p}$. 5. **Sample** discrete edge indicators $\mathbf{z}$ using Gumbel-Softmax with current $\tau$. 6. **Update
