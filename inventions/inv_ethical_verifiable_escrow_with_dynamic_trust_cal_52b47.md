...
The settlement process is executed via a hybrid off-chain/on-chain architecture within `contracts/EVEEscrow.sol`.
1. **Fund Locking**: Upon transaction initiation, the `lockFunds(uint256 amount, address recipient, uint16 TAS_Thresholds)` function is invoked, transferring the full escrow amount to a locked state on the blockchain, with the state initialized to 'Pending'.
2. **TAS Verification & Oracle Trigger**: An off-chain Oracle service continuously monitors the TAS computation. When the TAS
