...
Verification & Consensus Sub-protocol: Result validity is cryptographically proven using zk-SNARKs generated by the provider. The BFT committee aggregates these zk-SNARK proofs and, upon reaching consensus of validity, generates a signed attestation. This attestation is submitted to the smart contract, which then executes the state transition from 'COMPUTING' to 'SETTLED'. Weight updates for the credit system are managed by the same rotating BFT committee, which aggregates real-time performance sensor data to adjust W_reliability parameters, ensuring that credit valuations reflect current network health without requiring full-network consensus for every micro-transaction.
