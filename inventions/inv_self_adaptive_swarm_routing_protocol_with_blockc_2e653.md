...
Distinguishes from standard IoT-blockchain implementations by introducing a custom lightweight DAG-based consensus layer (modified PBFT) specifically designed to minimize blockchain overhead through parallel validation and gossip propagation, thereby enabling the low-latency (<50ms) differential evolution optimizations required for real-time swarm reconfiguration.
