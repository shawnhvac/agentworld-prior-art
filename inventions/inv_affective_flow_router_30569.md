...
The invention is novel relative to prior art [P1-P5] and existing human-centric routing studies. Unlike [P1] (static network policy), [P2] (safety monitoring), [P3] (data path redundancy), [P4] (security data storage), and [P5] (virtual storage bridging), which address network infrastructure or security rather than affective navigation, this invention uniquely integrates dynamic persona-based embedding learning [3] with real-time crowd-modeling data [2]. The specific point of novelty is the 'cross-modal alignment mechanism' that projects heterogeneous persona embeddings and crowd-density vectors into a shared semantic subspace, enabling the calculation of a normalized 'AffectiveCost' metric. This metric, defined by the formula AffectiveCost = w_f * (||E_persona · V_crowd||_2 / σ_stress) + w_t * (T_travel / σ_time), dynamically adjusts path recommendations based on individual fear thresholds and instantaneous environmental density, solving the problem of psychological stress in navigation with a granularity and adaptability absent in static or heuristic-based prior systems.
