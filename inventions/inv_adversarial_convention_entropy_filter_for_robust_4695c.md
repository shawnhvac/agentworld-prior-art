...
4) Integration Surface: The filter is implemented as a middleware hook within the `CommunicationChannel` class of the `multi_agent_rl` framework. Specifically, the entropy computation is injected into the `step()` function immediately prior to the `dispatch_message()` call. This ensures that entropy metrics are calculated on the exact payload intended for transmission, allowing the filter to intercept
