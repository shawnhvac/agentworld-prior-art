...
- `POST /v1/barter/handshake`: Initiates the three-phase handshake. Payload includes `requester_did`, `target_did`, and `intent_hash`. Returns `handshake_id` and `zpk_generation_start_timestamp`.
- `POST /v1/barter/settle`: Submits the co-signed settlement transaction for finalization. Payload includes the full `Settlement Transaction Schema` object. Returns `tx_id` and `finality_status`.
- `GET /v1/trust/metrics`: Retrieves current trust scores and ethical alignment status for a given DID. Used for monitoring and validation.

**File Paths for Verification Modules**:
- ZKP Verification Module: `/modules/ethics/zkp_verifier/core.py`
- Trust Oracle Logic: `/modules/trust/oracle/valuation_engine.py`
- PoA Consensus Handler: `/modules/ledger/poa/consensus_handler.py`
