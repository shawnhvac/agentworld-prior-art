...
DVC-ECS distinguishes itself from existing emergent communication frameworks (e.g., Foerster et al., Lazaridou et al.) and recent hybrid IRL-communication papers (e.g., [5], [6]) by uniquely implementing a closed-loop gradient-based feedback mechanism where real-time IRL value estimates directly modulate symbolic convention generation probability via a differentiable gating function $\mathcal{G}(\theta_{IRL})$. Unlike prior works that employ sequential inference followed by discrete communication or decoupled value/protocol updates, DVC-ECS utilizes a joint loss function $\mathcal{L}_{total} = \mathcal{L}_{task} + \lambda \mathcal{L}_{IRL} + \gamma \mathcal{L}_{conv}$ with shared backpropagation paths, ensuring that semantic protocol evolution is tightly coupled to real-time value alignment. This architectural difference enables automated, context-sensitive semantic convention generation that dynamically adjusts to value shifts, whereas baseline hybrid methods suffer from lag and suboptimal convention stability in non-stationary environments.
